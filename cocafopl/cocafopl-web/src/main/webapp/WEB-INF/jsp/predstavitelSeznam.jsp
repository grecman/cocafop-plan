			...
			<div class="mainAreaWide">
				<div class="formBar">
					<span style="margin-right: 0px;">&#160;Kalkulace:</span> <span style="margin-top: 3px; margin-left: 2px; margin-right: 2px;"> <a
						href="${pageContext.servletContext.contextPath}/srv/predstavitel/seznam/minusMesic"> <img
					...
					<c:choose>
						<c:when test="${empty mtk.gz39tKalkulace.schvaleno}">
							<span style="font-size: 16px; margin-left: 0px; margin-right: 0px;">${kalukaceRRRRMM}</span>
						</c:when>
						<c:otherwise>
							<span style="font-size: 16px; margin-left: 0px; margin-right: 0px; color: blue;" title="Schválená kalkulace">${kalukaceRRRRMM}</span>
						</c:otherwise>
					</c:choose>

					<span style="margin-top: 3px; margin-left: 2px; margin-right: 2px;"><a
						href="${pageContext.servletContext.contextPath}/srv/predstavitel/seznam/plusMesic"> <img
					...
					<c:if test="${(not(empty(kalukaceRRRRMM)))}">
						<c:choose>
							<c:when test="${(empty(mtk))}">
								<span> <form:form commandName="mt" action="${pageContext.servletContext.contextPath}/srv/predstavitel/seznam/vyberMt">
						&#160;MT a závod:
                            <form:select onchange="this.form.submit(); return true;" path="modelTr">
											<form:option value="0"> . . .  </form:option>
											<c:forEach var="i" items="${mtList}">
												<form:option value="${i.gz39tMt.modelTr}">${i.gz39tMt.modelTr}-${i.gz39tMt.zavod}</form:option>
											</c:forEach>
										</form:select>
									</form:form>
								</span>
							...
							<c:otherwise>
								<a href="${pageContext.servletContext.contextPath}/srv/predstavitel/seznam"> <SPAN>&#160;MT a závod:</SPAN><span
									style="color: #4BA82E; font-weight: bold; margin-left: 0px; margin-right: 0px;">${mtk.gz39tMt.modelTr}-${mtk.gz39tMt.zavod}</span>
								</a>
								...
								<c:if test="${(empty(mtk.gz39tKalkulace.schvaleno))}">
									<SPAN><a href="#openModalModelovyrok"><input type="button" value="Změnit" class="heroBtn" title="Změnit modelový rok"></input></a></SPAN>
								</c:if>
							</c:otherwise>
						</c:choose>
					</c:if>
				...
				<c:if test="${not empty pk}">
							...
							<tbody>
								<c:forEach items="${pk}" var="i">
									<tr>
										...
										<td align="left">${i.gz39tPredstavitel.modelovyKlic}</td>
										...
										<td align="center"><c:choose>
												<c:when test="${(i.gz39tPredstavitel.gz39tMt.platnostOd > i.gz39tPredstavitel.platnostOd)}">
										...
										<td align="center"><c:choose>
												<c:when test="${(i.gz39tPredstavitel.platnostDo > i.gz39tPredstavitel.gz39tMt.platnostDo)}">
													<SPAN style="background-color: red;" title="Platnost představitele je mimo rozsah platnosti modelové třídy!">
					...
					<div class="formBar">
						<c:if test="${(empty(mtk.gz39tKalkulace.schvaleno))}">
							<span><a href="${pageContext.servletContext.contextPath}/srv/predstavitel/seznam/favas"><input type="button" value="Prověřit MBV/Favas"
									class="heroBtn" style="width: auto;"></input></a></span>
						</c:if>
			...
			<div class="obalovak">
				...
				<div class="modalFooter">
					<SPAN style="color: red; font-weight: bold;">*</SPAN><SPAN style="font-size: x-small; color: gray;"> povinný údaj</SPAN><input type="button"
						id="formModelovyRokZmenaButton" value="Uložit" class="ok"></input>
				</div>
			</div>
		</div>
	</div>
