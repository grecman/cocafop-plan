...
public class KomunikaceController {
	...
	public String komunikaceFavas(@PathVariable long idMtKalk, @PathVariable long idPredKalk, @PathVariable String vseNeboZbytek, HttpServletRequest req, HttpSession session)
		...
		if (idMtKalk == 0 && idPredKalk > 0) {
			...
			log.debug("###\t komunikaceFavas(" + idMtKalk + ", " + idPredKalk + ") ... hromadna komunikace predstavitelu z obrazovky SEZNAM PREDSTAVITELU (" + vseNeboZbytek + ")");
	...
	@RequestMapping("/komunikaceFavas/mv")
	public String komunikaceFavasMv(UniObj uniObj, HttpServletRequest req, HttpSession session) throws InterruptedException, ParseException {
		log.debug("###\t komunikaceFavasMv()");
		...
		for (MvPopis mv : mvPopis) {

			VehicleOrder o = new VehicleOrder();

			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
			Date datum = sdf.parse(mv.getKalkulacniDatum());
			Calendar cal = Calendar.getInstance();
			cal.setTime(datum);
			o.setDate(cal);
			o.setNationalSalesProgramme(mv.getKodZeme());
			o.setModelCode(mv.getModelovyKlic());
			o.setModelYear(mv.getMrok());
			// o.setEquipment("PK22B1");
			
			if (mv.getVybavy() != null) {
				o.setEquipment(mv.getVybavy().replace("+", "").trim());
			}
			...
			for (int f = 1; f <= 3; f++) {
				if (o.getFavas() == null) {
					...
					Set<Equipment> equipment = favas.getEquipment();
					if (equipment.size() > 0) {
						log.debug("###\t\t equipment size: " + equipment.size()+"\t("+mv.getTypVozu()+" - "+mv.getModelovyKlic()+", "+req.getUserPrincipal().getName()+")");
						for (Equipment eq : equipment) {
							log.trace(eq.getFamily() + "-" + eq.getType() + "-" + eq.getValue());
							MvPopisPr pp = new MvPopisPr();
							pp.setRodina(eq.getFamily());
							pp.setTyp(eq.getType());
							pp.setPr(eq.getValue());
							pp.setUuser(req.getUserPrincipal().getName().toUpperCase());
							pp.setUtime(new Date());
							pp.setGz39tMvPopis(mv);
							serviceMvPopisPr.addMvPopisPr(pp);
						}
					}
					...
					if (message.size() > 0) {
						for (Message me : message) {
							log.trace("###\t\t message\t" + me.getCode() + "\t" + me.getText());
							MvPopisMessage pm = new MvPopisMessage();
							pm.setKod(me.getCode());
							pm.setText(me.getText());
							pm.setUuser(req.getUserPrincipal().getName().toUpperCase());
							pm.setUtime(new Date());
							pm.setGz39tMvPopis(mv);
							serviceMvPopisMessage.addMvPopisMessage(pm);
						}
					}
