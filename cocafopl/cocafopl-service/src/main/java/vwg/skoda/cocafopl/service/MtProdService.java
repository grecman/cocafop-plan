...
public class MtProdService {
	...
	public Iterable<String> findMtByString(String string) {
		...
		Iterable<String> gre = entityManager.createQuery("select u.modelovaTrida from MtProd u where u.modelovaTrida like :string ", String.class)
				.setParameter("string", "%" + string.toUpperCase() + "%").getResultList();
